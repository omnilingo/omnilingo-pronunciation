...
async function run_compare() {
    ...
    for(let row_i = 0; row_i < 25000; row_i ++) {
        let rand_i = Math.floor(Math.random() * (tsv.length-1))
        try {
            let stats = fs.statSync("../STT/data/en/clips/" + tsv[rand_i][1].slice(0, -3) + "wav");
            // console.log(stats)
        } catch (err) {
            skipped_files++;
            //console.log("skipping file " + "../STT/data/en/clips/" + tsv[rand_i][1].slice(0, -3) + "wav")
            continue;
        }
        let gold_phrase = clean_text(tsv[rand_i][2]);
        gold_filename = "../STT/data/en/json/" + tsv[rand_i][1] + ".json";
        const gold = JSON.parse(fs.readFileSync(gold_filename, 'utf8').replace(/\t/g, "\\t"));
        
        //console.log("Gold:")
        //console.log(gold)

        let rand_j = Math.floor(Math.random() * tsv.length);
        while (rand_j === rand_i || clean_text(tsv[rand_j][2]) === gold_phrase) {
            rand_j = Math.floor(Math.random() * tsv.length);
        }

        try {
            fs.statSync("../STT/data/en/clips/" + tsv[rand_j][1].slice(0, -3) + "wav");
        } catch (err) {
            continue;
        }
        filename = "../STT/data/en/json/" + tsv[rand_j][1] + ".json";
        ...
        let print_text = best_golds_print.join(',') + '\n' + best_tests_print.join(',') + '\n' +
            best_cos_print.join(',') + '\n' + best_jsd_print.join(',');
        let new_file = tsv[rand_i][1] + '.' + tsv[rand_j][1];
        console.log("writing to " + new_file);
        fs.writeFileSync('../STT/data/en/results_diff/' + new_file + '.txt', print_text);
